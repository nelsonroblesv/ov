...
<h2 class="text-2xl font-semibold mb-4 text-gray-900 dark:text-white">Listado de CFDIs</h2>
    ...
    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700:width:100%">
        <thead>
            <tr class="bg-indigo-600 dark:bg-indigo-800">
                ...
                <th class="px-6 py-3 text-left text-xs font-bold text-white uppercase tracking-wider">CfdiType</th>
        ...
        <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
            @forelse ($this->invoices as $invoice)
                <tr>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">{{ $invoice['Id'] ?? 'N/A' }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300">{{ $invoice['CfdiType'] ?? 'N/A' }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300">{{ $invoice['Type'] ?? 'N/A' }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ $invoice['Folio'] ?? 'N/A' }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ $invoice['Rfc'] ?? 'N/A' }}</td>
                </tr>
            @empty
                <tr>
